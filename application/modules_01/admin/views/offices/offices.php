...
function add_data(){
  window.location = "<?= base_url('admin/offices/form') ?>";
}
...
table = $('#table_data').DataTable({ 
      ...
      "order": [],
      "ajax": {
      "url": "<?php echo site_url('admin/offices/ajax_list')?>",
      "type": "POST"
      },
...
function update(id){
  window.location = "<?= base_url('admin/offices/form') ?>"+"/"+id;
}

function deletes(id){
  if(confirm('Delete ?')){
          $.ajax({
          url:"<?= base_url('admin/offices/delete_data') ?>",
          type:"POST",
          data:{"id":id},
          success:function(){
            reload_table();
