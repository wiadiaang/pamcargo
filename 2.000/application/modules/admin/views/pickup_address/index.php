      ...
      <div class="modal-body">
        <form action="javascript:void(0)" id="form_data" method="POST">
          <div class="form-group">
            ...
            <input type="hidden" name="pickup_address_id" class="form-control" >
...
var table;
 $(document).ready(function() {
	 table = $('#data_table').DataTable({ 
		 ...
		 "order": [], 
		 "ajax": {
			 "url": "<?php echo site_url('admin/pickup_address/ajax_list')?>",
			 "type": "POST"
		 },
		 "columnDefs": [
		 { 
...
function add_data()
{
	...
	$('#form_data').on('submit',function(){
        ...
        var user = $("[name='pickup_address_id']").val();
        if(user == ""){
           
                $.ajax({
                    url:"<?= base_url('admin/pickup_address/save_data');?>",
                    type:"POST",
 ...
 function hapus(no) {
  ...
  confirmButtonText: 'Yes, delete it!',
      ...
      if (result.value) {
                $.ajax({
                  type: "POST",
                  url: "<?= base_url('admin/pickup_address/delete') ?>",
                  data: {
                      'id': no
                  },
      ...
      function edit_data(id){
        ...
        $.ajax({
          url:"<?= base_url('admin/pickup_address/get_data/'); ?>"+id,
          type:"POST",
          dataType:"json",
          beforeSend:function(){
          ...
          success:function(data){
            $("[name='pickup_address_id']").val(data.pickup_address_id);
